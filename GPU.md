...
В чипе используется 7 графических процессорных кластеров (GPC), каждый кластер содержит 12 потоковых процессоров (Streaming Multiprocessor, SM-блоки), внутри каждого SM-блока есть 4 Warp-ядра и одно RT-ядро. Внутри каждого Warp-ядра находятся 32 CUDA-ядра (Shading Core) и одно Тензорное ядро.
...
- CUDA: простые вычисления, чаше всего выполняет операцию `A * B + C` за один такт
...
Можно посчитать, что GA 102 выполняет: 2 операции/такт * 10496 ядер * 1.7 Ghz = 35.6 миллиардов вычислений/секунду.
